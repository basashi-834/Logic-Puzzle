...
(function (global) {
  ...
  function loadStage(i) {
    ...
    renderStageList();
    pzBoard.render();
    if (pzBoard.circuit.wires.size === 0) {
      pzBoard.hint('線のつなぎ方：<b>端子をタップ → つなぎ先をタップ</b>', 'info');
    }
  }
  ...
  const HELP = '<h2>遊び方</h2>' +
    ...
    '<div><b>② 線でつなぐ（タップ 2 回）</b><p>つなぎ元の<b>端子をタップ</b>すると端子が光ります。つづけて<b>つなぎ先をタップ</b>すると線がつながります。つなぎ先は端子でなく<b>部品の本体をタップ</b>してもかまいません（空いている入力に自動でつながります）。もちろん<b>端子から端子へドラッグ</b>してもつながります。出力（右側）と入力（左側）だけがつながり、1 つの出力から何本にも分岐できます。</p></div>' +
    '<div><b>③ 線を消す</b><p><b>線をタップ</b>すると線が選ばれて赤い <b>✕</b> ボタンが出ます。それをタップすれば消えます。つながっている<b>入力側の端子を 2 回タップ</b>しても外れます（1 回目で選択、2 回目で解除）。パソコンでは線を選んで <b>Delete キー</b>、線の<b>右クリック</b>でも消せます。</p></div>' +
    '<div><b>④ スイッチを押す</b><p>入力ブロックの四角い 0 / 1 をクリックすると値が変わり、線と出力ランプの色がその場で変化します。</p></div>' +
    '<div><b>⑤ 部品を消す</b><p><b>部品をタップ</b>して出てくる <b>✕</b> ボタン、または部品を<b>ごみ箱へドラッグ</b>。パソコンでは<b>ダブルクリック</b>／<b>右クリック</b>でも消せます。ステージの入力スイッチ・出力ランプは消せません。</p></div>' +
